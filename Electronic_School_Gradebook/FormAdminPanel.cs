...
namespace Electronic_School_Gradebook
{
	public partial class FormAdminPanel : Form
	{
		...
		private void FormAdminPanel_Load(object sender, EventArgs e)
		{
			...
			{
				object countClasses = dBTools.executeAnySqlScalar("select count(ID_Class) from Classes;"); //используйте countRows
				object countStudents = dBTools.executeAnySqlScalar("select count(ID_Student) from Students;"); //используйте countRows
			...
			{
				object countClasses = dBTools.executeAnySqlScalar("select count(ID_Class) from Classes;"); //используйте countRows
				object countTeachers = dBTools.executeAnySqlScalar("select count(ID_Teacher) from Teachers;"); //используйте countRows
			...
			{
				//treeViewMainCommunications.Nodes.Clear();
				object countClasses = dBTools.executeAnySqlScalar("select count(ID_Class) from Classes;"); //нахуя? есть метод countRows


				nodeConnects = new NodeConnect[(int)countClasses /*нахуя? если есть treeClasses.Length или data.GetLength(0)*/];
				for (int i = 0; i < treeClasses.Length; i++)
				{
