...
namespace Electronic_School_Gradebook
{
	public partial class FormAdminPanel
	{
		...
		private void buttonRemoveRecordClick()
		{
			...
			if (tabControlAtoms.SelectedTab.Text == "Students")
			{
			...
			else if (tabControlAtoms.SelectedTab.Text == "Teachers")
			{
			...
			else if (tabControlAtoms.SelectedTab.Text == "Classes")
			{
				if (dataGridViewClasses.SelectedRows.Count > 0)
				{
					...
					if (result == DialogResult.Yes)
					{
						...
						dbTools.executeDelete("Classes", $"WHERE ID_Class = {classID}");
						FillDataGridViews();
					}
				}
			}
        ...
        private void FillDataGridViews(bool flag = false)
		{
			...
			switch (flag ? string.Empty : tabControlAtoms.SelectedTab.Text)
			{
				...
				case "Classes":
					...
					object[,] dataClasses = dBTools.executeSelectTable($"SELECT ID_Class, Name_Class, Type_Of_Class, Count_Students FROM Classes");
					classesRowConnect = new ClassesRowConnect[dataClasses.GetLength(0)];
					for (int i = 0; i < dataClasses.GetLength(0); i++)
					{
						dataGridViewClasses.Rows.Add(dataClasses[i, 1].ToString(), (bool)dataClasses[i, 2], dataClasses[i, 3].ToString());
						classesRowConnect[i].id = (int)dataClasses[i, 0];
						classesRowConnect[i].rowIndex = i;
					}
					break;
                case "Students":
		...
		private void RefreshAllThisDataGridView()
		{
			DBFormsTools dbFormsTools = new DBFormsTools(FormAuthorization.sqlConnection);

			dbFormsTools.FillDGV(ref dataGridViewUsers, "Users");

			dbFormsTools.FillDGV(ref dataGridViewStudents, "Students");

			dbFormsTools.FillDGV(ref dataGridViewTeachers, "Teachers");
			...
			dbFormsTools.FillDGV(ref dataGridViewParents, "Parents");

			dbFormsTools.FillDGV(ref dataGridViewSubjects, "Subjects");
		}
        ...
        private void dataGridViewUsersCellEndEdit()
        {
            ...
            if (columnName == "Role_User" && !System.Text.RegularExpressions.Regex.IsMatch(newWriting.ToString(), @"^[a-zA-Z]+$"))
            {
                ...
                MessageBox.Show("Only Latian characters are allowed for the Role of user field.");
            }
            else
            {
        ...
        private void dataGridViewClassesCellEndEdit()
		{
			...
			else
			{
				string valueString = $"{columnName}='{newWriting}'";
				string conditionString = string.Empty;
				for (int i = 0; i < classesRowConnect.Length; i++)
				{
					if (classesRowConnect[i].rowIndex == selectRow)
					{
						conditionString = $"WHERE ID_Class = {classesRowConnect[i].id}";
						break;
					}
				}
